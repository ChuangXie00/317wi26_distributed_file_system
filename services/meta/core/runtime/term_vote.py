...
# 本地观测到更高 term 时统一执行降级逻辑，避免旧 leader/candidate 继续参与写入或选举。
def step_down_to_follower(term: int, reason: str, *, sync_epoch: bool = True, reset_voted_for: bool = True) -> bool:
    with _RUNTIME_LOCK:
        ...
        # 进入新 term 后清空旧 leader 视图，等待后续 heartbeat/coordinator 收敛新的 leader。
...
# 显式推进 term 并按需降级，供 quorum 入站/对账链路复用。
...
# 显式记录任期投票对象，统一处理 stale-term 与投票落账。
...
# 观测到更高任期时推进 term，并按需同步 epoch（用于后续 quorum 入站处理）。
