...
# 中文：运行时状态锁；保护 role/leader/epoch/lamport 的并发读写一致性。
_RUNTIME_LOCK = threading.RLock()


# 中文：返回当前 UTC 时间的 ISO 字符串，统一调试字段格式。
def _now_iso() -> str:
    return datetime.now(timezone.utc).isoformat().replace("+00:00", "Z")


# 中文：规范化角色值，避免非法字符串污染运行态。
...
# 中文：推进 Lamport 逻辑时钟；收到远端时钟时先做 max，再 +1。
...
# 中文：进入 candidate 轮次，epoch 自增并清空当前 leader。
...
# 中文：收到更高 epoch 的 election 消息时先降级并刷新 epoch，避免旧主继续写入。
...
# 中文：观测 leader 信息（heartbeat/coordinator），并执行 fencing 降级规则。
def observe_leader(leader_id: str, leader_epoch: int, reason: str) -> Dict[str, Any]:
    ...
    with _RUNTIME_LOCK:
        ...
        if incoming_epoch < current_epoch:
            ...
            # 中文：同 epoch 冲突时采用确定性规则（字典序较大 leader 胜出）避免双主。
